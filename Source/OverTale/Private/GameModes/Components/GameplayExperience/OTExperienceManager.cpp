...
UOTExperienceManager::UOTExperienceManager(const FObjectInitializer& ObjInit)
	: Super(ObjInit)
{
...
void UOTExperienceManager::CallOrRegister_OnExperienceLoaded_HighPriority(FOnExperienceLoaded::FDelegate&& Delegate)
{
...
void UOTExperienceManager::CallOrRegister_OnExperienceLoaded(FOnExperienceLoaded::FDelegate&& Delegate)
{
	if (IsExperienceLoaded())
	{
		Delegate.Execute(CurrentExperience);
...
void UOTExperienceManager::CallOrRegister_OnExperienceLoaded_LowPriority(FOnExperienceLoaded::FDelegate&& Delegate)
{
	if (IsExperienceLoaded())
	{
		Delegate.Execute(CurrentExperience);
...
const UOTExperienceDefinition* UOTExperienceManager::GetCurrentExperienceChecked() const
{
...
bool UOTExperienceManager::IsExperienceLoaded() const
{
	return (LoadState == EExperienceLoadState::Loaded) && (CurrentExperience != nullptr);
}

void UOTExperienceManager::StartExperienceLoad()
{
	check(CurrentExperience != nullptr);
	...
	for (const TObjectPtr<UOTExperienceActionSet>& ActionSet : CurrentExperience->ActionSets)
	{
	...
	//@TODO: Centralize this client/server stuff into the OTAssetManager
...
void UOTExperienceManager::OnExperienceLoadComplete()
{
	...
	if (NumGameFeaturePluginsLoading > 0)
	{
		...
		{
			UOTExperienceHandler::NotifyOfPluginActivation(PluginURL);
...
void UOTExperienceManager::OnGameFeaturePluginLoadComplete(const UE::GameFeatures::FResult& Result)
{
...
void UOTExperienceManager::OnExperienceFullLoadCompleted()
{
	check(LoadState != EExperienceLoadState::Loaded);
...
void UOTExperienceManager::OnActionDeactivationCompleted()
{
...
void UOTExperienceManager::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
	...
	for (const FString& PluginURL : GameFeaturePluginURLs)
	{
		if (UOTExperienceHandler::RequestToDeactivatePlugin(PluginURL))
		{
	...
	if (LoadState == EExperienceLoadState::Loaded)
	{
		...
		{
			UE_LOG(LogTemp, Error, TEXT("Actions that have asynchronous deactivation aren't fully supported yet in OverTale experiences"));
		}
...
void UOTExperienceManager::OnAllActionsDeactivated()
{
